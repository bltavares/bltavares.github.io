...
** Trazendo o mesmo conceito(?) de possível falha para iniciar uma sessão
   ...
   Diferente de criar um pedido, as razões de erro podem ser inúmeras: podemos ter um erro ao fazer o parsing do /JSON/, ou a nossa conexão cair.
...
** Invalidando uma ordem depois que ela é enviada
   ...
   O token deveria ser compartilhado ao fazer vários pedidos. O pedido precisa não pode ser compartilhado (não entendi essa frase!).
   Nesse caso eu gosto de pensar que o pedido foi "consumido" por /send_order/, invalidando qualquer outra tentativa de utilizar a estrutura por outros meios (quais outros meios).
...
** Trazendo uma resposta sobre o resultado do Pedido
   ...
   O compilador sabe que /Result/ tem a semanta (seria semântica?) de uma operação que pode falhar, e nos avisa se não utilizamos o valor. Obrigado =rustc=.
