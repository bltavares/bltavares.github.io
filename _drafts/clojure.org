...
** Voltando a pergunta sobre as diferenças de criar funções
   ...
   ~(fn função [arg] arg)~ é uma variação de ~fn~ que dá um nome a função.
   ...
   O exemplo acima cria um alerta em 5 segundos, e registra outro
   chamada para executar de novo no final.
