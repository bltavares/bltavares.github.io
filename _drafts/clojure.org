...
** Outras formas de criar escopos
   ...
   Quando nos chamamos a ~funcao~, as expressões que vamos executar
...
** Voltando a pergunta sobre as diferenças de criar funções
   ...
   Na maior parte do tempo que você quiser criar uma funcao com um
   ...
   Casos de recursão da mesma funcao fazem melhor uso de ~recur~ ao
   inves de funcao nomeada. Com ~recur~, nos vamos evitar de chegar no
   limite máximo de chamadas de funcao (StackOverflow)
